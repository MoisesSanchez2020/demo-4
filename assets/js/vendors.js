...
jQuery(document).ready(function($){
	...
	function initHeadline() {
		...
		singleLetters($('.wc-headline.letters').find('b'));
		//initialise headline animation
		animateHeadline($('.wc-headline'));
	}
	...
	function animateHeadline($headlines) {
		...
		$headlines.each(function(){
			...
			if(headline.hasClass('loading-bar')) {
				...
				setTimeout(function(){ headline.find('.wc-words-wrapper').addClass('is-loading') }, barWaiting);
			} else if (headline.hasClass('clip')){
				var spanWrapper = headline.find('.wc-words-wrapper'),
					newWidth = spanWrapper.width() + 10
				spanWrapper.css('width', newWidth);
			} else if (!headline.hasClass('type') ) {
				//assign to .wc-words-wrapper the width of its longest word
				var words = headline.find('.wc-words-wrapper b'),
					width = 0;
				words.each(function(){
					var wordWidth = $(this).width();
				    if (wordWidth> width) width = wordWidth;
				});
				headline.find('.wc-words-wrapper').css('width', width);
			};
	...
	function hideWord($word) {
		...
		if($word.parents('.wc-headline').hasClass('type')) {
			var parentSpan = $word.parent('.wc-words-wrapper');
		...
		} else if($word.parents('.wc-headline').hasClass('letters')) {
			var bool = ($word.children('i').length>= nextWord.children('i').length) ? true : false;
			hideLetter($word.find('i').eq(0), $word, bool, lettersDelay);
			showLetter(nextWord.find('i').eq(0), nextWord, bool, lettersDelay);

		}  else if($word.parents('.wc-headline').hasClass('clip')) {
			$word.parents('.wc-words-wrapper').animate({ width : '2px' }, revealDuration, function(){
				switchWord($word, nextWord);
				showWord(nextWord);
			});

		} else if ($word.parents('.wc-headline').hasClass('loading-bar')){
			$word.parents('.wc-words-wrapper').removeClass('is-loading');
			switchWord($word, nextWord);
			setTimeout(function(){ hideWord(nextWord) }, barAnimationDelay);
			setTimeout(function(){ $word.parents('.wc-words-wrapper').addClass('is-loading') }, barWaiting);
	...
	function showWord($word, $duration) {
		if($word.parents('.wc-headline').hasClass('type')) {
			...
			$word.parents('.wc-words-wrapper').animate({ 'width' : $word.width() + 10 }, revealDuration, function(){ 
	...
	function showLetter($letter, $word, $bool, $duration) {
		...
		if(!$letter.is(':last-child')) { 
			...
			if($word.parents('.wc-headline').hasClass('type')) { setTimeout(function(){ $word.parents('.wc-words-wrapper').addClass('waiting'); }, 200);}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!countdownEl) return; // prevent error if countdown not on page

  // Always set target to 20 days from now
  const now = new Date().getTime();
  const targetDate = now + (20 * 24 * 60 * 60 * 1000); // 20 days in ms

  const circles = document.querySelectorAll(".progress");
  const totalCircumference = 2 * Math.PI * 50; // 314
  ...
  function updateCountdown() {
    ...
    if (distance <= 0) {
      countdownEl.innerHTML = "<h2>We're live!</h2>";
      clearInterval(timer);
      return;
    }
    ...
    document.getElementById("days").innerHTML = `${days}<span class="time-unit">Days</span>`;
    document.getElementById("hours").innerHTML = `${hours}<span class="time-unit">Hours</span>`;
    document.getElementById("minutes").innerHTML = `${minutes}<span class="time-unit">Minutes</span>`;
    document.getElementById("seconds").innerHTML = `${seconds}<span class="time-unit">Seconds</span>`;

    // Update progress bars
    circles[0].style.strokeDashoffset = totalCircumference - (days / 20) * totalCircumference;
    circles[1].style.strokeDashoffset = totalCircumference - (hours / 24) * totalCircumference;
    circles[2].style.strokeDashoffset = totalCircumference - (minutes / 60) * totalCircumference;
    circles[3].style.strokeDashoffset = totalCircumference - (seconds / 60) * totalCircumference;
  }
...
var Dots = (function() {
    ...
    var generate = function ( params ) {
    	...
    	options.selector = (typeof options.selector === "string") ? options.selector : '.moving-particles';
...
class Cursor {
    ...
    bind() {
        ...
        }).on('mouseenter', 'a,input,textarea,button,.progress-wrap', () => {
            self.setState('-pointer');
        }).on('mouseleave', 'a,input,textarea,button,.progress-wrap', () => {
